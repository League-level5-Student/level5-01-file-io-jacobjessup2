	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < pixel.length; i++) {
			for (int j = 0; j < pixel.length; j++) {
				...
				g.fillRect(pixel[i][j].x*pixelWidth, pixel[i][j].y*pixelHeight, pixelWidth, pixelHeight);
				g.setColor(Color.black);
				g.drawRect(pixel[i][j].x*pixelWidth, pixel[i][j].y*pixelHeight, pixelWidth, pixelHeight);
			}
		}
	}
}
